...
export function AtusWidget() {
    ...
    const BusItem = ({ bus, stopId }: { bus: BusArrival, stopId: string }) => {
        const isArriving = bus.sureDakika === -1;

        return (
            <div className="group flex items-center justify-between p-2.5 rounded-lg bg-muted/30 border border-border/50 hover:bg-muted/50 transition-all duration-300">
                ...
                <div className={`grid grid-cols-1 ${activeStop.showOfis && activeStop.showEv ? 'md:grid-cols-2' : 'md:grid-cols-1'} gap-6`}>
                    ...
                    {activeStop.showOfis && (
                        <div className="space-y-2.5">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 xl:grid-cols-2 gap-1.5">
                                {ofisGidis.length > 0 ? (
                                    ofisGidis.map((bus, i) => <BusItem key={`ofis-${i}`} bus={bus} stopId={activeStop.id} />)
                                ) : (
                                    <div className="col-span-full text-[11px] text-muted-foreground/60 italic p-4 text-center border border-dashed rounded-lg bg-muted/5">
                    ...
                    {activeStop.showEv && (
                        <div className="space-y-2.5">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 xl:grid-cols-2 gap-1.5">
                                {eveDonus.length > 0 ? (
                                    eveDonus.map((bus, i) => <BusItem key={`ev-${i}`} bus={bus} stopId={activeStop.id} />)
                                ) : (
                                    <div className="col-span-full text-[11px] text-muted-foreground/60 italic p-4 text-center border border-dashed rounded-lg bg-muted/5">
